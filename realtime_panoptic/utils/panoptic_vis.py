...
def masked_image(predictions, original_image, label_id_to_names, fade_weight=0.8):
...
def panoptic_visualization(pred1, pred2, input_image):
    ...
    y_instance = seg_image(pred2, input_image, cm_instance, 0)
    y_seg = seg_image(pred2, input_image, cm_seg, 0.5)

    x = masked_image(pred1, input_image, cityscapes_instance_label_name)
    ...
    mask1 = cv2.inRange(x[1]*255, lower, upper)
